...
export default function ReactionEditorModal({
  ...
  const handleSubmit = () => {
    ...
    onClose({ type: 'question', comment: finalComment, x: pos.x, y: pos.y })
  }
      ...
      <div style={styles.container}>
        ...
        <div style={styles.canvas}>
          ...
          >
            ...
            {pos && (
              <div style={{ ...styles.marker, left: `${pos.x * 100}%`, top: `${pos.y * 100}%` }}>
                <HelpCircle size={32} fill="#4D96FF" stroke="#000" strokeWidth={1.5} />
              </div>
            )}
          </div>
        </div>
...
const styles: { [key: string]: React.CSSProperties } = {
  ...
  marker: { position: 'absolute', transform: 'translate(-50%, -50%)', zIndex: 10, pointerEvents: 'none' },
  ...
  controls: { flexShrink: 0, padding: '20px 16px calc(24px + env(safe-area-inset-bottom))', background: '#111', borderTop: '1px solid rgba(255,255,255,0.1)' },
