...
export default function AnswerActionBar({ problemId, reactionCount, onPreviewChange }: any) {
  ...
  useEffect(() => {
    ...
    // stepやopenが変わった後にDOMが作られるため、依存配列に入れて監視
  }, [step, open])
    ...
    <div style={{ position: 'static' }}>
      ...
      {open && step === 'input' && (
        <div style={{
          ...
          left: `${pos.x * 100}%`,
          top: `${pos.y * 100}%`,
          transform: 'translate(-50%, -130%)', // ピンの真上に浮かせる
          ...
          <div style={styles.boxHeader}>
            <span style={{ fontSize: '10px', color: '#999', fontWeight: 600 }}>アイコンをドラッグして移動</span>
            <X size={16} color="#ccc" onClick={() => {setOpen(false); onPreviewChange(null);}} style={{ cursor: 'pointer' }} />
          </div>
...
const styles: { [key: string]: CSSProperties } = {
  ...
  inputBox: { position: 'absolute', width: '240px', background: '#fff', border: '1px solid #ddd', borderRadius: '16px', padding: '12px', boxShadow: '0 10px 40px rgba(0,0,0,0.2)', zIndex: 3000, display: 'flex', flexDirection: 'column', gap: 10, willChange: 'left, top' },
  ...
  input: { flex: 1, background: 'transparent', border: 'none', outline: 'none', fontSize: '16px', padding: '10px 45px 10px 12px', color: '#333' },
  sendIconBtn: { position: 'absolute', right: '4px', background: '#333', color: '#fff', border: 'none', borderRadius: '8px', padding: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }
}