          ...
          parts: [
            { inlineData: { mimeType: 'image/jpeg', data: base64Image } },
            {
              text: `
                ...
                3. 推論（inference）ノードと定理の判定（操作説明と定理ノードの枝分かれ）:
                   - メインの推論ノード: 生徒が次の命題を導くための論理・計算を具体的に言語化してください。「右辺の式を簡略化し、展開する」などの操作内容や、使用した「公式・定理名」を簡潔に記述します。
                   - 定理の分岐ノード（新規追加）: 推論において定理や公式が使われた場合、メインの推論ノードから枝分かれする形で新しいノードを作成してください。この分岐ノードの type は「inference」とし、label には「定理名：数式・定義の内容」を具体的に記述してください。
                   - 【重要】生徒が答案に定理や公式の名前を明記していなくても、数式の変形過程から公式・定理が使われたことが明らかな場合は、公式の利用を推測して特定し、メインの推論ノードと分岐ノードを作成してください。
                  ...
                  
                {
                  "graph": {
                    "nodes": [
                      ...
                      { "id": "t1", "label": "自然数の和の公式: Σ_{k=1}^{n} k = n(n+1)/2", "type": "inference" },
