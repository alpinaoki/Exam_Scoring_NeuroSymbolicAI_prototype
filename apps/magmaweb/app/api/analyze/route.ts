            ...
            { inlineData: { mimeType: 'image/jpeg', data: base64Image } },
            {
              text: `
                ...
                3. 推論（inference）ノードと定義・定理（theorem）ノードの接続ルール:
                   ...
                   - 【特例ルール（命題からの直接接続）】推論の内容が公式変換に直接関係ない場合や、命題の数式自体が定義・定理に直接依存している場合は、「命題ノード」から直接「定義・定理ノード」へエッジを接続して出力することを許可・推奨します。
                   - 【分岐（並行展開）の明示】1つの命題から2つ以上の推論に分岐する場合（例：場合分け、または1つの式から複数の条件を同時に導く場合）は、それぞれの推論ラベルの先頭に「[場合分け1]」「[条件A]」などのプレフィックスをつけ、数学的な並行展開であることを明示してください（YES/NOのフローチャートと混同させないため）。
