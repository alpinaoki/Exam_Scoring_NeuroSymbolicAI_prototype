            ...
            { inlineData: { mimeType: 'image/jpeg', data: base64Image } },
            {
              text: `
                ...
                3. 推論（inference）ノードと定理（theorem）ノードの分離:
                   ...
                   - 命題に伴う定理の分岐ノード: 命題ノードに書かれている数式自体に、特定の定義や定理、公式が使われている・含まれている場合（例：式の中にΣ記号や特定の公式の形が含まれる場合など）、その【命題ノードから直接枝分かれする形】で新しいノードを作成してください。この分岐ノードの type も「theorem」とし、label には「定理名・定義名：数式・定義の内容」を具体的に記述して、該当する命題ノードからエッジを繋げて枝分かれさせてください。
                   - 【重要】生徒が答案に定理や公式の名前を明記していなくても、数式の変形過程や記述された式そのものから公式・定理が使われたことが明らかな場合は、公式の利用を推測して特定し、適切な分岐ノード（type: theorem）を作成してください。
                ...
                5. 論理の飛躍と暗算の補完:
                   - 途中式が省略されており論理や計算の飛躍がある場合は、その間を埋める操作を明記し、メイン推論ラベルの先頭に「[推測]」と付けてください。（例：「[推測] 自然数の和の公式を利用し、右辺の式を簡略化して展開する」）
                6. グラフや表の除外（ノイズ処理）:
