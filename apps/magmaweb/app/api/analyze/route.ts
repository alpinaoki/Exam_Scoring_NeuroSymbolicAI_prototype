          ...
          parts: [
            { inlineData: { mimeType: 'image/jpeg', data: base64Image } },
            {
              text: `
                ...
                3. 推論（inference）ノードと定理の判定（操作説明と多角的な定理ノードの枝分かれ）:
                   - メインの推論ノード: 生徒が次の命題を導くための論理・計算を具体的に言語化してください。「右辺の式を簡略化し、展開する」などの操作内容や、使用した「公式・定理名」を簡潔に記述します。
                   - 変形に伴う定理の分岐ノード: 推論（変形）において定理や公式が使われた場合、メインの推論ノードから枝分かれする形で新しいノードを作成してください（typeは inference とし、labelに 定理名：数式・定義の内容 を記述）。
                   - 命題に伴う定理の分岐ノード（新規追加）: 命題ノードに書かれている数式自体に、特定の定義や定理、公式が使われている・含まれている場合（例：式の中にΣ記号や特定の公式の形が含まれる場合など）、その【命題ノードから直接枝分かれする形】で新しいノードを作成してください。この分岐ノードの type は「inference」とし、label には「定理名・定義名：数式・定義の内容」を具体的に記述して、該当する命題ノードからエッジを繋げて枝分かれさせてください。
                   - 【重要】生徒が答案に定理や公式の名前を明記していなくても、数式の変形過程や記述された式そのものから公式・定理が使われたことが明らかな場合は、公式の利用を推測して特定し、適切な分岐ノードを作成してください。
                ...
                5. 論理の飛躍と暗算の補完:
                   - 途中式が省略されており論理や計算の飛躍がある場合は、その間を埋める操作を明記し、メイン推論ラベルの先頭に「[推測]」と付けてください。（例：「[推測] シグマの公式を利用し、右辺の式を簡略化して展開する」）
                6. グラフや表の除外（ノイズ処理）:
                   - 解答用紙に描かれている関数グラフ、幾何的な図形、増減表などの表は解析の対象外とします。ノードとして抽出しないでください。
                7. 忠実性の原則:
                   - 生徒が答案に明記した Graves「誤った数式」自体を正しい数式に修正・補正することは厳禁です。誤った数式はそのまま「命題」ノードとして抽出してください。
                  ...
                  
                {
                  "graph": {
                    "nodes": [
                      ...
                      { "id": "t1", "label": "総和記号(Σ)の定義: 数列の和を簡易的に表す記号", "type": "inference" },
                      { "id": "i2", "label": "[推測] 自然数の和の公式を利用し、右辺の式を簡略化して展開する", "type": "inference", "applied_theorem": "自然数の和の公式", "is_in_library": false },
                      { "id": "t2", "label": "自然数の和の公式: Σ_{k=1}^{n} k = n(n+1)/2", "type": "inference" },
